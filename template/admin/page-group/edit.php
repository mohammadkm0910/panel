...
<?php require_once BASE_DIR . "/template/admin/partial/navbar-admin.php"; ?>
    <h5 class="container-fluid mt-2">بروز کردن گروه</h5>
    <form action="<?php url('page-group/update/'.$id); ?>" method="post" class="p-2 mt-1" id="form-group">
        <div class="mt-2">
            <label for="title">عنوان گروه</label>
            <input type="text" class="form-control mt-2" id="title" name="title" placeholder="عنوان گروه را وارد کنید" value="<?php if ($groupCount > 0) echo $pageGroup['title'] ?>">
        </div>
        <div class="m2">
        ...
        <script>
            $("#parent_id").val("<?php echo $pageGroup['parent_id']; ?>").change();
        </script>
        <input type="submit" class="btn btn-warning mt-2 mr-60px" value="ذخیره">
    </form>
    <div class="line"></div>
