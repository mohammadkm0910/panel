...
<?php require_once BASE_DIR . "/template/admin/partial/navbar-admin.php"; ?>
    <h5 class="container-fluid mt-2">بروز کردن صفحه</h5>
    <form action="<?php url('page/update/'.$id); ?>" method="post" enctype="multipart/form-data" class="p-2 m-2" id="form-page">
        <div class="mt-3">
        ...
        <div class="mt-2">
            ...
            <input type="text" class="form-control" id="title" name="title" placeholder="عنوان صفحه را وارد کنید" value="<?php echo $joinPage['title'][0]; ?>">
        </div>
        <div class="mt-2">
            <label for="summary" class="form-label">توضیح مختصر</label>
            <textarea type="text" class="form-control" id="summary" name="summary" placeholder="توضیح مختصر صفحه را وارد کنید"><?php echo $joinPage['summary']; ?></textarea>
        </div>
        <div class="mt-2">
            <label for="body" class="form-label">متن صفحه</label>
            <textarea type="text" class="form-control" id="body" name="body" placeholder="متن صفحه خود را وارد کنید"><?php echo $joinPage['body']; ?></textarea>
        </div>
        <div class="mt-2">
            <label class="form-label" for="image">تصویر</label>
            <input type="file" class="form-control" id="image" name="image">
            <img class="img-thumbnail img-small mt-2" src="<?php asset($joinPage['image']); ?>" alt="">
        </div>
        <div class="mt-2">
        ...
        <div class="d-flex mt-2">
            ...
            <div class="form-check form-switch mt-1 mr-8px">
                <input class="form-check-input" type="checkbox" id="slider" name="slider" <?php echo $joinPage['slider']? "checked" : ""; ?>>
            </div>
        </div>
        <input type="submit" class="btn btn-warning mt-3 mr-60px" value="ذخیره">
        <script>
            $("#group_id").val("<?php echo $joinPage['group_id']; ?>").change();
            $("#status").val("<?php echo $joinPage['status']; ?>").change();
        </script>
    </form>
    <div class="line"></div>
